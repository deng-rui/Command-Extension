...
public class Vote {
		...
		switch(playerpick){
			...
			case "cy" :
				if (authorityControl(playerplayer,"votec")) {
					if (isteam) {
						endYesMsg = () -> com.github.dr.extension.core.ex.Extend.addMesgTeam(team,"vote.cy.end",playerplayer.name,type+" "+(isBlank(name)?"":name));
                    } else {
                        endYesMsg = () -> com.github.dr.extension.core.ex.Extend.addMesgAll("vote.cy.end",playerplayer.name,type+" "+(isBlank(name)?"":name));
                    }
					require = 0;
					forceEnd();
				} else {
			...
			case "cn" :
				if (authorityControl(playerplayer,"votec")) {
					if (isteam) {
                        endNoMsg = () -> com.github.dr.extension.core.ex.Extend.addMesgTeam(team,"vote.cn.end",playerplayer.name,type+" "+(isBlank(name)?"":name));
                    } else {
                        endNoMsg = () -> com.github.dr.extension.core.ex.Extend.addMesgAll("vote.cn.end",playerplayer.name,type+" "+(isBlank(name)?"":name));
                    }
					playervote = 0;
					forceEnd();
				} else {
	...
	private void normalDistribution() {
		...
		endNoMsg = () -> com.github.dr.extension.core.ex.Extend.addMesgAll("vote.done.no",type+" "+(isBlank(name)?"":name),playervote,temp);
		endYesMsg = () -> com.github.dr.extension.core.ex.Extend.addMesgAll("vote.ok");
		start(() -> com.github.dr.extension.core.ex.Extend.addMesgAll("vote.start",player.name,type+" "+(isBlank(name)?"":name)));
	}
	...
	private void teamOnly() {
		...
		require = temp;
		endNoMsg = () -> com.github.dr.extension.core.ex.Extend.addMesgTeam(team,"vote.done.no",type+" "+(isBlank(name)?"":name),playervote,temp);
		endYesMsg = () -> com.github.dr.extension.core.ex.Extend.addMesgTeam(team,"vote.ok");
		start(() -> com.github.dr.extension.core.ex.Extend.addMesgTeam(team,"vote.start",player.name,type+" "+(isBlank(name)?"":name)));
	}
	...
	private void start(Runnable run){
		...
		countDown=Data.SERVICE.scheduleAtFixedRate(
			...
			public void run() {
				...
				com.github.dr.extension.core.ex.Extend.addMesgAll("vote.ing",reciprocal);
			}
	...
	private void kick() {
		com.github.dr.extension.core.ex.Extend.addMesgAll("kick.done",name);
	...
	private void host() {
		...
		com.github.dr.extension.core.ex.Extend.loadmaps(true, () -> world.loadMap(result, result.applyRules(gamemode)),gamemode);
	}
